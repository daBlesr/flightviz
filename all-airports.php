...
<body>
  ...
  .defer(d3.json, "query.php?q=all-airports")
  .defer(d3.json, "query.php?q=airports&a=20")
  .await(loadFlightData);

  function loadFlightData(error, world, countryData, airports, topUSAAirports){
    $.post("query.php?q=flights-by-airport",{
        "airports": topUSAAirports.map(function(airport) {
          return airport.airport;
        })
      }, 
      function( data ) {
        ready(error, world, countryData, airports, topUSAAirports, JSON.parse(data));
    });
  }

  //Main function

  function ready(error, world, countryData, airports, topUSAAirports, flights) {
    ...
    airports.forEach(function(d) {
      airportById[d.id] = d.name;
      option = airportList.append("option");
      option.text(d.city + ": " + d.name);
      option.property("value", d.id);
    });


    var m = 0;
    for(var i = 0; i < topUSAAirports.length; i++){
      if(topUSAAirports[i].total_traffic > m) m = topUSAAirports[i].total_traffic;
    }
    ...
    var circles_svg = svg.selectAll("circle")
      ...
      .append("circle")
      .attr("class","airport")
      .attr("r", function (d) {return "2px"; }).style("fill", "red")
    ...
    function redraw(){
      ...
      svg.selectAll("circle")
        ...
        .attr("opacity", function(d) {
          ...
          var geoangle = d3.geo.distance(x,[-p[0],p[1]]);
